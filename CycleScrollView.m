...
- (void)loadPageWithContent:(NSInteger)index atIndex:(NSInteger)page;
@end
...
{
    NSInteger length = abs((int)index - (int)currentIndex);
...
{
    int page = (first + ITEM_COUNT/2 +1)%(int)[pageViews count];
    ((UILabel*)pageViews[page]).textColor = [UIColor blackColor];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    {
        [self.delegate scrollToIndex:(int)[self currentPage]];
    }
}
