...
@property (nonatomic,readwrite) NSUInteger activeTabIndex;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    UIImage *tabImage = [UIImage imageNamed:@"未标题-4.png"];
    UIImageView *tabImageView = [[UIImageView alloc] initWithImage:tabImage];
    tabImageView.frame = CGRectMake(0, 0, 64, 32);
    _tabBackImage = tabImageView;
}
...
//    frame.size.height = CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) - (self.tabBarController.tabBar.hidden ? 0 : CGRectGetHeight(self.tabBarController.tabBar.frame));
    frame.size.height = CGRectGetHeight(self.view.frame) - (self.tabBarController.tabBar.hidden ? 0 : CGRectGetHeight(self.tabBarController.tabBar.frame)) -17 ;
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    //将图片放置于文字之下
    [tabView sendSubviewToBack:_tabBackImage];
    //调整图片位置，将图片居中
    [_tabBackImage mas_makeConstraints:^(MASConstraintMaker *make){
        make.center.equalTo(tabView);
    }];
    
    //这里控制当移到最后几个tab的时候不会出现空白
    if ((_tabCount - activeTabIndex) <= 2) {
        UIView *last3View = [self tabViewAtIndex:(_tabCount - 2)];
        CGRect last3ViewFrame = last3View.frame;
        last3ViewFrame.origin.x += (CGRectGetWidth(last3ViewFrame) / 2);
        last3ViewFrame.origin.x -= CGRectGetWidth(self.tabsView.frame) / 2;
        last3ViewFrame.size.width = CGRectGetWidth(self.tabsView.frame);
        
        if (last3ViewFrame.origin.x < 0) {
            last3ViewFrame.origin.x = 0;
        }
        
        if ((last3ViewFrame.origin.x + CGRectGetWidth(last3ViewFrame)) > self.tabsView.contentSize.width) {
            last3ViewFrame.origin.x = (self.tabsView.contentSize.width - CGRectGetWidth(self.tabsView.frame));
        }
...
//        self.tabsView.contentSize = CGSizeMake(400, 50);
    ...
    //在边边上添加一个button
    UIButton *nextTatButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [nextTatButton setBackgroundImage:[UIImage imageNamed:@"list_news_top_arrow_normal@2x.png"] forState:UIControlStateNormal];
    [nextTatButton setBackgroundImage:[UIImage imageNamed:@"list_news_top_arrow_press@2x.png"] forState:UIControlStateHighlighted];
    [nextTatButton addTarget:self action:@selector(touchNextTabButton) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:nextTatButton];
    [nextTatButton mas_makeConstraints:^(MASConstraintMaker *make){
        make.size.mas_equalTo(CGSizeMake(28, 45));
        make.right.equalTo(self.tabsView);
        make.top.equalTo(self.tabsView);
    }];
    ...
    //这里将width加了28.0，以留出nextbutton的位置
    self.tabsView.contentSize = CGSizeMake(contentSizeWidth + 28.0, [self.tabHeight floatValue]);
...
//这里自己添加一个nextTabButton点击方法
- (void)touchNextTabButton{
    NSUInteger now = self.activeTabIndex;
    NSUInteger next = now + 1;
...
//        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, 40, 40)];
